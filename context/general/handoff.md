...
**Last session:** 2026-05-22 — Phase 9 (Tracking) shipped end-to-end. Meta Pixel + Conversions API dual-fire wired for 3 events (ViewContent, AddToCart, InitiateCheckout) across 3 PDPs + Buybox + cart drawer. PageView auto-fires from Pixel base. Purchase fires from Shopify's Facebook & Instagram channel app on shop.shmocard.com (D-04 — never from our code). GHL order webhook configured Admin-side per D-05 (zero site code). Shared UUIDv4 event_id pairs browser Pixel + server CAPI for Events Manager dedup (48h window). Pixel ID `1279390273687409` parity across both domains. Phase 9 ships INDEPENDENT of Vercel deploy. Race-condition fix shipped on ViewContent / AddToCart fires (`next/script afterInteractive` can leave `window.fbq` undefined briefly after hydration → poll with 5s budget). Plus mid-phase fix: `/shmo-review` category-page Buybox now wired to live CR-80 Shopify product (was using DEFAULT_BUYBOX_PACKS, ATC broken). Pre-Phase-9 polish session also shipped 8 logical units in 6 commits: gallery pagination, image cap bump, free-shipping progress band, cart reminder shrink, FormatCompare unify + ember badge white, mobile 2-col format grid, mobile horizontal-scroll video carousel, L-Sign color × qty variant matrix with image swatches.

Memory rules saved this session: (a) DO NOT run `npm run build` while dev server is running — corrupts `.next/` cache, breaks all routes. Only run build at phase close-out and clear `.next/` after.

---

## Project phase

**Phase 9 — Tracking ✅ complete 2026-05-22.** Site fires browser Pixel + server CAPI for ViewContent (PDP mount), AddToCart (Buybox.handleAdd success), InitiateCheckout (CartCheckoutButton click BEFORE Shopify redirect). PageView auto from Pixel base. Shopify channel app handles Purchase + checkout-side events on shop.shmocard.com. Verified live via Playwright network capture + Jordan-side Pixel Helper (ad blocker had initially blocked the verification). 10 atomic commits in Phase 9. See `.planning/phases/09-tracking-ghl-webhook-facebook-pixel/09-SUMMARY.md`.

**Next phase: Phase 10 — Launch readiness.** Final a11y audit, Vercel project env var setup (NEXT_PUBLIC_FB_PIXEL_ID + FB_CAPI_ACCESS_TOKEN — NOT FB_TEST_EVENT_CODE per Pitfall 5), Shopify revalidation webhook (now possible with public Vercel URL — was deferred from Phase 8), DNS cutover from `shop.shmocard.com` to `shmocard.com`. Highest blast radius — last phase. Gated on full design + tracking sign-off in localhost dev. Run **`/gsd-plan-phase 10`** to kick off.

**Project progress:** 9 of 10 phases complete (90%). Phases 1, 2, 3a, 3, 4, 5, 6, 7, 8, 9 done. Remaining: 10 (Launch readiness — Vercel env + DNS cutover + Shopify revalidation webhook).
