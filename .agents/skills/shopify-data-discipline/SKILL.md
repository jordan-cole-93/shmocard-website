...
Auto-triggers when Codex is touching anything related to product data, pricing, inventory, or cart/checkout.
...
| Typography, colors, radii, shadows, motion | Code | `.shm-*` classes + tokens from `.agents/skills/shmocard-design-system/colors_and_type.css` (per `.claude/rules/design-system.md`) |
...
- A pre-tool-use hook in `.codex/hooks.json` validates Shopify-related Bash commands aren't destructive.
