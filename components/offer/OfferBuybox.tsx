...
// components/offer/OfferBuybox.tsx — Buybox for the free + shipping funnel.
// Forked from components/shmo-review/Buybox.tsx — see .planning/sidequests/free-shipping-funnel/SPEC.md § 5.
//
// Differences from the canonical Buybox:
//   1. Default pack idx = 0 (FREE row) instead of 3 (10-pack).
//   2. FREE row renders "FREE" + "+ $7 shipping" instead of "$0.00".
//   3. Free-shipping progress band removed (offer fixes shipping at $7).
//   4. CTA label adapts to selected pack.
//
// Same design-system primitives, same Section wrapping, same ATC wiring +
// Pixel/CAPI dual-fire as Buybox.tsx.
...
export default function OfferBuybox({
  ...
  ariaLabel = "Claim your free Shmo Review card",
  nextBg = "marsh",
}: BuyboxProps) {
  ...
  const [packIdx, setPackIdx] = useState(0); // FREE pack default (first row)
            ...
            {currentPacks.map((p, i) => (
              ...
              >
                ...
                <span className="shm-pack-row__price">
                  {p.price === 0 ? (
                    <>
                      <span className="shm-pack-row__price-now">FREE</span>
                      <span className="shm-pack-row__price-meta">+ $7 shipping</span>
                    </>
                  ) : (
                    <>
