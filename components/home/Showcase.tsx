...
const GLOW_STYLE: React.CSSProperties = {
  background:
    "radial-gradient(ellipse at center, rgba(255,91,31,0.40), rgba(255,184,51,0.18) 30%, transparent 65%)",
  filter: "blur(20px)",
};
...
export default function Showcase() {
  ...
  return (
    <section
      aria-label="The Shmocard family"
      className="relative bg-cocoa-deep text-paper overflow-hidden"
    >
      <div
        aria-hidden="true"
        className="absolute -top-12 left-1/2 -translate-x-1/2 w-[1100px] max-w-full h-[600px] pointer-events-none z-0"
        style={GLOW_STYLE}
      />

      <div className="relative z-10 max-w-[1320px] mx-auto px-7 py-9">
        <div className="text-center max-w-[720px] mx-auto">
          <span className="inline-flex items-center gap-3">
            <span
              aria-hidden="true"
              className="block w-[22px] h-px bg-honey opacity-50"
            />
            <span className="font-serif italic font-normal text-[15px] text-honey">
              The Shmocard family
            </span>
          </span>
          <h2 className="mt-3 text-[44px] lg:text-[52px] leading-[1.02] tracking-[-0.03em] font-semibold text-paper text-balance">
            Four tools.{" "}
            <em className="font-serif italic font-normal text-honey">One</em>{" "}
            toolkit.
          </h2>
          <p className="mt-4 text-[17px] leading-[1.55] text-[rgba(255,235,210,0.78)] max-w-[56ch] mx-auto">
            Shmo Review is shipping. Three more sub-brands roll out through
            next year — same cards, same dashboard, more jobs they can do.
          </p>
        </div>

        <div className="text-center mt-7">
          <ol className="inline-flex items-center gap-x-8 gap-y-3 flex-wrap rounded-lg border border-[rgba(255,235,210,0.14)] bg-[rgba(255,248,234,0.04)] px-6 py-4">
            <li className="flex items-center gap-3 flex-wrap">
              {ROADMAP.map((node, i) => (
                <span key={node.sublabel} className="flex items-center gap-3">
                  {i > 0 && (
                    <span
                      aria-hidden="true"
        ...
        <div
          ...
          className="mt-7 rounded-2xl border border-[rgba(255,235,210,0.14)] bg-[rgba(255,248,234,0.04)] p-7 lg:p-9"
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-7 lg:gap-9 items-center">
            <div key={brand.id} className="flex flex-col items-start text-left">
              ...
              <h3 className="text-[32px] lg:text-[40px] leading-[1.04] tracking-[-0.025em] font-semibold text-paper text-balance">
                {brand.headlinePre}
              ...
              <div className="mt-6 w-full flex flex-wrap gap-2">
                {brand.status === "live" ? (
                  <a
                    href={brand.ctaHref}
                    className="inline-flex items-center gap-2 h-12 px-5 rounded-md bg-ember text-paper font-semibold hover:bg-ember-deep transition-colors active:scale-[0.98]"
                  >
                    {brand.ctaLabel}
                  ...
                  >
                    ...
                    <button
                      type="submit"
                      className="inline-flex items-center gap-2 px-5 rounded-md bg-ember text-paper font-semibold hover:bg-ember-deep transition-colors active:scale-[0.98]"
                    >
                      Notify
            ...
            <div className="lg:pl-2">
              <div
                ...
                className="relative w-full max-w-[520px] mx-auto lg:ml-auto lg:mr-0 rounded-xl border border-[rgba(255,235,210,0.18)] bg-ember-soft overflow-hidden"
                style={{
                  aspectRatio: "4/3",
                  backgroundImage:
              ...
              >
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 px-6 text-center">
                  <Camera size={56} strokeWidth={1.5} className="text-ember-deep opacity-50" />
                  <span className="text-[11px] font-semibold tracking-[0.08em] uppercase text-ember-deep">
                    {brand.visualLabel}
                  </span>
                </div>
              </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors ${
                  isActive
                    ? "bg-ember border border-ember text-paper hover:bg-ember-deep hover:border-ember-deep"
              ...
              >
                ...
                <span className="flex flex-col leading-tight min-w-0">
                  ...
                  <span className="text-[11.5px] text-[rgba(255,235,210,0.6)] truncate">
                    {b.role}
                  </span>
                </span>
              </button>
