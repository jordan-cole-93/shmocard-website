...
Run the `design-system-auditor` sub-agent (Sonnet, defined at `.claude/agents/design-system-auditor.md`) against a given file to verify design-system compliance before commit.
...
- Don't audit files in `.claude/skills/shmocard-design-system/` (those ARE the design system).
