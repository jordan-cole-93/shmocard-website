...
Audit the design system at `.claude/skills/shmocard-design-system/`, surface structural / naming / consistency issues, decide integration strategy with Next.js + Tailwind 4, resolve open questions with Jordan, and lock the canonical files Phase 3 will consume. **No code changes in this phase** — output is decision artifacts only.

## Success criteria

1. Every file in `.claude/skills/shmocard-design-system/` accounted for in the audit doc.
...
**Goal:** Comprehensive inventory of `.claude/skills/shmocard-design-system/`. Every file read, every concern flagged.
...
1. **Folder location:** propose final location for the design system folder. Options:
   - Keep at `.claude/skills/shmocard-design-system/` (rename to kebab → `.claude/skills/shmocard-design-system/`)
...
**Tasks:**
1. Add a section to root `CLAUDE.md` linking to `.claude/skills/shmocard-design-system/CLAUDE.md` as the design-system-scoped ruleset (when working in that subtree).
2. Add a section to `.claude/skills/shmocard-design-system/CLAUDE.md` linking back to root `CLAUDE.md` for repo-level rules (file organization, live-store protection, Shopify data discipline).
...
3. Decide approach. Two real options:
   - **A: Direct CSS import.** `app/globals.css` does `@import "tailwindcss"; @import "../.claude/skills/shmocard-design-system/colors_and_type.css"; @import "../.claude/skills/shmocard-design-system/components.css";`. CSS stays the source of truth. Tailwind 4 utilities used only for layout (grid, flex, padding, gap) — never for color, type, radius, shadow.
...
1. Folder rename — keep `.claude/skills/shmocard-design-system/` (with space) or rename to kebab?
...
1. Update root `CLAUDE.md` "Where to find things" table:
   - Replace "Design system: _Pending — folder not yet dropped..._" with concrete paths to `.claude/skills/shmocard-design-system/CLAUDE.md`, `SKILL.md`, `PRIMITIVES.md`, `colors_and_type.css`, `components.css`.
...
4. Decide on `DESIGN.md` / `PATTERNS.md` at repo root:
   ...
   - If Jordan disagrees, write minimal pointer files at root that say "see `.claude/skills/shmocard-design-system/CLAUDE.md`".
...
- **Folder rename ripple effect.** If we rename `.claude/skills/shmocard-design-system/` → `.claude/skills/shmocard-design-system/`, every internal CSS path / HTML reference inside the design system itself needs updating (relative `fonts/...` paths, `<link rel="stylesheet" href="...">`, etc.). Mitigation: confirm all internal refs are relative-to-folder-root before renaming; do rename + ref update in a single atomic step.
